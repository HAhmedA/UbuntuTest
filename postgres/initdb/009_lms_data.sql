...
-- 
-- KEY DESIGN: Each student has 4 subjects (subject_1 to subject_4)
-- All metrics are per-subject, per-day

-- =============================================================================
-- LMS SESSIONS (Raw Data - one record per subject per day)
-- =============================================================================
CREATE TABLE IF NOT EXISTS public.lms_sessions (
  id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id uuid NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
  
  -- Subject identifier (1-4)
  subject_id int NOT NULL CHECK (subject_id >= 1 AND subject_id <= 4),
  ...
  -- One session per user per subject per date
  CONSTRAINT unique_lms_session UNIQUE (user_id, subject_id, session_date)
);

-- Indexes for efficient queries
CREATE INDEX IF NOT EXISTS idx_lms_sessions_user ON public.lms_sessions (user_id);
CREATE INDEX IF NOT EXISTS idx_lms_sessions_user_subject ON public.lms_sessions (user_id, subject_id);
CREATE INDEX IF NOT EXISTS idx_lms_sessions_user_date ON public.lms_sessions (user_id, session_date DESC);
CREATE INDEX IF NOT EXISTS idx_lms_sessions_user_subject_date ON public.lms_sessions (user_id, subject_id, session_date DESC);

-- =============================================================================
-- LMS BASELINES (Rolling averages per subject)
-- =============================================================================
CREATE TABLE IF NOT EXISTS public.lms_baselines (
  ...
  -- One baseline per user per subject
  CONSTRAINT unique_lms_baseline UNIQUE (user_id, subject_id)
);

CREATE INDEX IF NOT EXISTS idx_lms_baselines_user ON public.lms_baselines (user_id);
CREATE INDEX IF NOT EXISTS idx_lms_baselines_user_subject ON public.lms_baselines (user_id, subject_id);

-- =============================================================================
-- LMS JUDGMENTS (Two sentences per subject per time window)
-- =============================================================================
CREATE TABLE IF NOT EXISTS public.lms_judgments (
  ...
  -- One judgment per user per subject per period
  CONSTRAINT unique_lms_judgment UNIQUE (user_id, subject_id, period_start, period_end)
);

-- Indexes for chatbot lookups
CREATE INDEX IF NOT EXISTS idx_lms_judgments_user ON public.lms_judgments (user_id);
CREATE INDEX IF NOT EXISTS idx_lms_judgments_user_subject ON public.lms_judgments (user_id, subject_id);
